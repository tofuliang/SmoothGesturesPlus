!function () {
    ...
    var a = function () {
        ...
        var n = 336 - Math.ceil((Date.now() - e.firstinstalled) / 1e3 / 60 / 60);e.license = 'full';
        e.license && $("#trialperiod").css({display: "none"}), $("#expirein").append($("<span>").css({
            "background-color": n < 0 ? "rgba(255,0,0,.2)" : n < 120 ? "rgba(255,255,0,.2)" : "rgba(0,255,0,.2)",
            "font-weight": "bold"
        }).text("Your trial period " + (n < 0 ? "has expired" : "will expire in " + (n >= 24 ? Math.round(n / 24) + " days" : n > 0 ? n + " hours" : "less than an hour")))), (e.hideNoteRemindRate || -1 == ["full", "1yrmul"].indexOf(e.license)) && $(".note_remindrate").css({display: "none"}), $(".note_remindrate .close").click(function () {
        ...
        $(".page[page=custom] .pagetitle").append($("<div id=addaction class='button gray'>").html("<span>+</span> " + s("options_button_addcustomaction")).on("click", u)), v()
    }, i = function () {
                    ...
                    try {
                        ...
                        (function (n) {
                            try {
                                ...
                                }(n = n.substring(n.indexOf("{"), n.lastIndexOf("}") + 1));
                                var s = JSON.parse(n);
                                if ("Smooth Gestures Settings" != s.title) return !1;
                                s.gestures && (e.gestures = s.gestures);
                                for (var a in s.settings) -1 != o.indexOf(a) && (e[a] = s.settings[a]);
                                return t(e), alert("Import Successful"), !0
                            } catch (e) {
                            }
                            return !1
                        })(a) || alert("Import Failed")
                    }
                    if (i) {
        ...
        e.customactions[s] = n, t({customactions: e.customactions}), v(), setTimeout(p.bind(null, s), 500)
    }, d = function (n) {
        ...
        $("#customedit").remove(), $(".page[page=custom] .action").css({display: ""}), a.css({display: "none"}), a.after($("<div id=customedit>").append($("<div class='button gray customsave' tabindex=0>").text("save").on("click", function () {
            s.title = $(".customtitle").val(), s.descrip = $(".customdescrip").val(), s.code = $(".customcode").val(), s.context = $(".customcontext").val(), e.customactions[n] = s, t({customactions: e.customactions}), $("#customedit").remove(), $(".page[page=custom] .action").css({display: ""}), v()
    ...
    var m = function (n) {
        ...
        chrome.runtime.getBackgroundPage(function (t) {
            for (l = 0; l < h.length; l++) {
                i = $(".page[page=" + h[l].id + "]");
                $(".action", i).remove();
                for (var n = 0; n < h[l].actions.length; n++) {
                    var a = h[l].actions[n], o = t.contexts[a];
                    $(".actiongroup.disabled", i).append($("<div class=action>").attr("action", a).attr("sectionindex", n).append("<div class=gestures>").append($("<div class='button gray addactiongesture' tabindex=0>+</div>").on("click", g.open.bind(null, a))).append(o ? $("<img class=context>").attr("src", "/img/icon-" + ("l" == o ? "link" : "i" == o ? "image" : "s" == o ? "selection" : "") + ".png") : null).append($("<div class=headtitle>").text(s("action_" + a))).append($("<p class=sub>").text(s("descrip_" + a))).append("<div class=clear>"))
                }
            }
            var i = $(".page[page=custom]");
            $(".action", i).remove();
            var l = 0;
            for (var a in e.customactions) {
                var c = e.customactions[a], o = t.contexts[a];
                $(".actiongroup.disabled", i).append($("<div class=action>").attr("action", a).attr("sectionindex", l).append($("<div class='button gray delcustomaction' tabindex=0>&times;</div>").on("click", d.bind(null, a))).append($("<div class='button gray editcustomaction' tabindex=0>edit</div>").on("click", p.bind(null, a))).append("<div class=gestures>").append($("<div class='button gray addactiongesture' tabindex=0>+</div>").on("click", g.open.bind(null, a))).append(o ? $("<img class=context>").attr("src", "/img/icon-" + ("l" == o ? "link" : "i" == o ? "image" : "s" == o ? "selection" : "") + ".png") : null).append($("<div class=headtitle>").text(c.title)).append($("<p class=sub>").text(c.descrip)).append("<div class=clear>")), l++
            }
            b()
        })
    }, b = function () {
        ...
        }, 1e3), "full" == e.license ? ($("#note_licensestatus .title").html("You have the full license!"), $("#note_licensestatus").addClass("green").removeClass("yellow"), $("#noplus").css({display: "none"})) : "1yrmul" == e.license ? ($("#note_licensestatus .title").html("You have the 1 year license"), $("#note_licensestatus").addClass("green").removeClass("yellow")) : "6mnmul" == e.license ? ($("#note_licensestatus .title").html("You have the 6 month license"), $("#note_licensestatus").addClass("green").removeClass("yellow")) : "1mnmul" == e.license ? ($("#note_licensestatus .title").html("You have the 1 month license"), $("#note_licensestatus").addClass("green").removeClass("yellow")) : "2wkmul" == e.license ? ($("#note_licensestatus .title").html("You have the 2 week license"), $("#note_licensestatus").addClass("green").removeClass("yellow")) : "1wkmul" == e.license ? ($("#note_licensestatus .title").html("You have the 1 week license"), $("#note_licensestatus").addClass("green").removeClass("yellow")) : "1yr1cl" == e.license ? ($("#note_licensestatus .title").html("You have the 1 year / 1 install license"), $("#note_licensestatus").addClass("green").removeClass("yellow")) : "1mn1cl" == e.license ? ($("#note_licensestatus .title").html("You have the 1 month / 1 install license"), $("#note_licensestatus").addClass("green").removeClass("yellow")) : "1wk1cl" == e.license ? ($("#note_licensestatus .title").html("You have the 1 week / 1 install license"), $("#note_licensestatus").addClass("green").removeClass("yellow")) : e.license ? ($("#note_licensestatus .title").html("You have an unknown license"), $("#note_licensestatus").addClass("yellow").removeClass("green"), $("#nolicense").css({display: "block"})) : ($("#note_licensestatus .title").html("You do not have an active license"), $("#note_licensestatus").addClass("yellow").removeClass("green")), e.license_expires) {
